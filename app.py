...
# Matching Section
    ...
    with col1:
        st.subheader("For Jobs")
        job_id = st.number_input("Enter Job ID", min_value=1, value=1)
        if st.button("Find Candidate Matches"):
            job = session.query(Job).get(job_id)
            if job:
                candidates = session.query(Candidate).all()
                matches = [(c, calculate_matching_score(job, c)) for c in candidates]
                matches.sort(key=lambda x: x[1], reverse=True)
    ...
    with col2:
        ...
        candidate_id = st.number_input("Enter Candidate ID", min_value=1, value=1)
        if st.button("Find Job Matches"):
            candidate = session.query(Candidate).get(candidate_id)
            if candidate:
                jobs = session.query(Job).all()
                matches = [(j, calculate_matching_score(j, candidate)) for j in jobs]
                matches.sort(key=lambda x: x[1], reverse=True)
